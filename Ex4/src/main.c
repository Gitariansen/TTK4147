...
int main()
{
	...
	xTaskCreate( taskA, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
	xTaskCreate( taskB, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
	xTaskCreate( taskC, (signed char * ) "BASIC", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL);
...
void taskA()
{
	const delayTime = 1 / portTICK_RATE_MS;
	const delay5 = 5 / portTICK_RATE_MS;
	...
	while(1){
		...
		vTaskDelayUntil(&xLastWakeTime, delay5);
	}
}
...
void blinkLed0()
{
	const delayTime = 200 / portTICK_RATE_MS;
	while(1)
	{
		gpio_toggle_pin(LED0_GPIO);
		printf("tick\n");
...
void blinkLed1()
{
	const delayTime = 500 / portTICK_RATE_MS;
	while(1)
	{
